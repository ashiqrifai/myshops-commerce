...
export const metadata:Metadata={title:"New Arrivals | MyShops",description:"Shop new arrivals at MyShops UAE.",alternates:{canonical:"/products/new-arrivals"}};
...
const find=(ss:StorefrontSection[])=>ss.find(s=>{const c=(s.content||{}) as Content,t=norm(c.title);return norm(s.type?.code)==="product_carousel"&&(t==="new arrivals"||t==="new arrivals"||t.includes("new arrival"))})||null;
...
const pageUrl=(q:Record<string,string|string[]|undefined>,n:number)=>{const p=new URLSearchParams();Object.entries(q).forEach(([k,v])=>{if(k==="page"||v===undefined||v==="")return;p.set(k,Array.isArray(v)?v.join(","):v)});if(n>1)p.set("page",String(n));const x=p.toString();return x?`/products/new-arrivals?${x}`:"/products/new-arrivals"};
